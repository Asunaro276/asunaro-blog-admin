...
5. **実装プロセスの実行**
   ...
   - 実施者: requirements-refiner
   - タスクの詳細要件を記述
   - 受け入れ基準を明確化

   b. **テストケース作成** (`tdd-testcases.md`)
   - 実施者: tdd-test-generator
   - 単体テストケースを作成
   - エッジケースを考慮

   c. **テスト実装** (`tdd-red.md`)
   - 実施者: tdd-test-implementer
   - 失敗するテストを実装
   - テストが失敗することを確認

   d. **最小実装** (`tdd-green.md`)
   - 実施者: minimal-implementation-engineer
   - テストが通る最小限の実装
   - 過度な実装を避ける

   e. **リファクタリング** (`tdd-refactor.md`)
   - 実施者: code-refactoring-engineer
   - コードの品質向上
   - 保守性の改善

   f. **品質確認** (`tdd-verify-complete.md`)
   - 実施者: qa-code-reviewer
   - 実装の完成度を確認
   - 不足があれば c-f を繰り返す
   ...
   a. **準備作業の実行**
   - 実施者: repo-infrastructure-engineer
   - ディレクトリ作成
   - 設定ファイル作成
   - 依存関係のインストール
   - 環境設定
   ...
   b. **作業結果の確認**
   - 実施者: infrastructure-qa-evaluator
   - 作業完了の検証
   - 期待された成果物の確認
   - 次のタスクへの準備状況確認
